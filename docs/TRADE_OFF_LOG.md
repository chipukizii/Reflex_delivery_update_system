...
## 4. Order identifiers are derived by scanning existing records

**What it is.** New order ids are generated by finding the highest
existing id and incrementing it. This guarantees uniqueness in a single
process, but it reads the whole order table on every create and would
break under concurrent writes, where two requests could compute the same
next value.

**Acceptable because.** The prototype is single process with a small
number of orders, so both the scan cost and the race are theoretical.
The earlier random approach could silently overwrite a live order, which
was the failure actually worth removing.

**With more time.** Let the datastore issue identifiers, either a
database sequence or a UUID, so uniqueness does not depend on reading
what already exists.
